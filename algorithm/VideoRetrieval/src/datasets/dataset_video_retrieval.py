...
class HDVILAVideoRetrievalDataset(Dataset):
    ...
    def __init__(self, cfg, vis_dir, anno_path, vis_format='video', mode="train"):
        ...
        self.n_clips = cfg.train_n_clips if mode == "train" else cfg.test_n_clips  # 1
        self.num_frm = cfg.train_num_frms if mode == "train" else cfg.test_num_frms  # 12
        self.sample_rate = cfg.sample_rate  # 0
        if hasattr(cfg, "text_pos_num"):
            self.pos_num = cfg.text_pos_num
        else:
            self.pos_num = 1
    ...
    def id2path(self, id):
        ...
        if self.vis_format == 'video':
            name = os.path.join(self.vis_dir,
                                clip_name + ".mp4")  # clip_name.split('/')[-1]+".mp4") # .../3fps_video/ +
    ...
    def get_sample_idx(self, total_frame_num):
        ...
        if self.sample_rate > 0:
            ...
            if hasattr(self.cfg,
                       "sample_jitter") and self.cfg.sample_jitter and self.mode == "train":  # sample_jitter = 1
                interval = int(total_frame_num / (self.n_clips * self.num_frm - 1))  # 间隔数量
    ...
    def load_frames(self, vis_path, total_frame_num):
        ...
        img_array = []
        for i in frame_idx:
            img = Image.open(os.path.join(vis_path,  # path为一个视频对应目录 \
                                          vis_path.split('/')[-1] + '_{0:03d}.jpg'.format(i + 1))).convert("RGB")
    ...
    def __getitem__(self, index):
        ...
        if isinstance(texts, list):
            texts = random.sample(self.datalist[index]['text'], self.pos_num)  # 随机在多个query中pos_num个
            if 'didemo_data' in self.anno_path or 'actnet_data' in self.anno_path:
        ...
        return dict(
            ...
            texts=texts,
            # vis_id = torch.tensor([int(vis_id[5:])])
        )


class VideoEmbedDataset(Dataset):

    def __init__(self, cfg, vis_dir, anno_path=None, vis_format='video', mode="train"):
        assert vis_format in ["video", "frame"]
        self.cfg = cfg
        self.vis_dir = vis_dir
        self.anno_path = anno_path
    ...
    def __getitem__(self, index):
        ...
        vis_path = self.datalist[index]
        video = self.load_video(vis_path) if self.vis_format == 'video' else self.load_frames(
            vis_path, self.datalist[index]['num_frame']
        )
...
class VideoEmbedCollator(object):
...
class TextEmbedCollator(object):
    def __init__(self, tokenizer, max_length=40, is_train=True):
        self.tokenizer = tokenizer
        self.max_length = max_length
        self.is_train = is_train
    ...
    #             texts = random.sample(self.datalist[index]['text'], self.pos_num) # 随机在多个query中pos_num个
    #         else:
    #             texts = random.sample(self.datalist[index]['text'], 1) # 随机在多个query中pos_num个
...
class VideoRetrievalCollator(object):
    ...
    def collate_batch(self, batch):
        if isinstance(batch[0]["video"], torch.Tensor):
            ...
            v_collate = img_collate
        video = v_collate([d["video"] for d in batch])
        # vis_id = v_collate([d["vis_id"] for d in batch])
