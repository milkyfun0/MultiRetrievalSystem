...
# 初始化分布式进程组
dist.init_process_group(backend='nccl')  # backend 可以是 'nccl', 'gloo' 或 'mpi'
...
def validate(model, val_loaders, cfg, device, cur_best_t2v=None):
    ...
    for loader_name, val_loader in val_loaders.items():
        ...
        for val_step, batch in enumerate(val_loader):
            ...
            # vis_patch_feat = allgather(feats['vis_features'].unsqueeze(1), cfg)
            # vis_id = allgather(batch['vis_id'], cfg).squeeze().tolist()
        ...
        # concat all vis_ids
        # vis_ids = ['video'+str(id) for b_id in vis_ids for id in b_id]
        # vis_ids = vis_ids[:valid_len]
        ...
        # LOGGER.info(f'size of sim matrix: {len(sim_matrix_)}, size of sim proxy: {len(sim_proxy)}')
...
def start_eval():
    ...
    # total_train_batch_size = int( # 2*64=128
    ...
    #     if cfg.if_tb_log:
    #         TB_LOGGER.create(join(cfg.output_dir, 'log.txt'))
    ...
    #
    #     if cfg.if_log2file:
    #         add_log_to_file(join(cfg.output_dir, "log.txt", "log.txt.txt"))
    # else:
    #     LOGGER.disabled = True
    #     # pbar = NoOp()
    #     model_saver = NoOp()
    ...
    #
    #     if hasattr(model, 'module'):
    #         torch.clamp_(model.module.clipmodel.logit_scale.data, 0, np.log.txt(200))
    #         logit_scale_ = model.module.clipmodel.logit_scale.data
    #     else:
    #         torch.clamp_(model.clipmodel.logit_scale.data, 0, np.log.txt(200))
    ...
    #     #         param.grad.data /= cfg.world_size
    #
    #     # optimizer
