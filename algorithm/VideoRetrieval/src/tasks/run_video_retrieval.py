...
def init_device(args, local_rank):
    ...
    device = torch.device("cpu" if torch.cuda.is_available() else "cpu", local_rank)
        ...
        for idx1, b_text in enumerate(text_feats):
            ...
            for idx2, b_ in enumerate(zip(vis_patch_feats, vis_feats)):
                ...
                if hasattr(model, 'module'):  # we do not implement this without nvidia apex
                    text_proxies = model.module.text_proxy(b_text, b_vis_patch,
                                                           b_vis_patch)  # (a,b,dim): each text of a generates b proxies with b videos
...
def start_training():
    ...
    val_loaders = {k: PrefetchLoader(v, None)
                   for k, v in val_loaders.items()}

    return model, tokenizer, train_loader, val_loaders["msrvtt-1ka"]
    # print(val_loaders)
    # for val_step, batch in enumerate(val_loaders["msrvtt-1ka"]):
    #     print(val_step, batch)
    #     break
    # img_norm = None
