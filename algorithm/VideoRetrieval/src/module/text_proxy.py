...
from src.module.text_wass import CrossAttentionStack
from collections import OrderedDict
from src.module.text_wass import QuickGELU
...
class Proxy_Layer(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        tgt2, atten_weights = self.multihead_attn(tgt, memory,
                                                  memory, )  # tgt2: (num_proxy, B, 512), weights: (B, 1, 12, 12*12)
...
class TextProxy(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.proxy_decoder = CrossAttentionStack(num_layers=2, d_model=512, nhead=2, dim_feedforward=512)
        ...
        # Prototypes for text quality scoring
        self.num_prototype = getattr(config, 'num_prototype', 16)
        self.text_quality = nn.Parameter(torch.randn(self.num_prototype, d_model))
        nn.init.xavier_uniform_(self.text_quality)

        # MLPs for prototype-based quality
        self.text_mlp1 = nn.Sequential(OrderedDict([
            ("fc1", nn.Linear(self.num_prototype, d_model)),
            ("gelu", QuickGELU()),
            ("dropout", nn.Dropout(dropout)),
            ("fc2", nn.Linear(d_model, d_model))
        ]))
        self.text_mlp2 = nn.Linear(d_model, 1)

        # Learnable global scale
        self.alpha = nn.Parameter(torch.tensor(0.0))

        # Init weights
        for module in (self.text_mlp1, self.text_mlp2):
        ...
        # Number of proxies sampled per text
        self.k_sample = getattr(config, 'k_sample', 2)

    def alpha_activate(self, x):
        # return torch.clamp(torch.exp(-x), min=0, max=1)
        return torch.sigmoid(-x)

    def text_feature_quality(self, text_feat: torch.Tensor) -> torch.Tensor:
        """
        Compute a quality score per text feature against learnable prototypes.
        Returns shape (B, 1).
        """
        protos = F.normalize(self.text_quality, p=2, dim=-1)  # (P, D)
        feats = F.normalize(text_feat, p=2, dim=-1)  # (B, D)
        sims = feats @ protos.t()  # (B, P)
        similarity = sims
        # for i in range(len(similarity)):
        #     print(f"text {i}", similarity[i])

        x = self.text_mlp1(sims)  # (B, D)
        scale = self.text_mlp2(x) * self.alpha  # (B, 1)
        scale = self.alpha_activate(scale)
        return scale
    ...
    def forward(self, text_feat, video_feat, frame_feat, is_train=False, step=None):
        """
            ...
            frame: (b,m,512)
        """
        text_scale = self.text_feature_quality(text_feat)

        if is_train:
            text_feat = allgather(text_feat.contiguous(), self.cfg)
            video_feat = allgather(video_feat.contiguous(), self.cfg)
            text_scale = allgather(text_scale.contiguous(), self.cfg)
            # frame_feat = allgather(frame_feat.contiguous(), self.cfg)

        if self.training:
            pro_proxies = self.proxy_decoder(text_feat, video_feat[:, :self.cfg.using_M, :], k_sample=self.k_sample)
        else:
            pro_proxies = self.proxy_decoder(text_feat, video_feat[:, :self.cfg.using_M, :], k_sample=self.k_sample * 8)

        pro_proxies = pro_proxies.permute(1, 0, 2)  # ->(a,b,512)
        k_sample = pro_proxies.shape[0] // text_feat.shape[0]
        text_feat = text_feat.unsqueeze(1).repeat(1, k_sample, 1)
        ...
        text_proxy = text_feat.unsqueeze(1) + dir_vec
        # text_proxy = dir_vec delete text_feature is  not work

        # (a,1,512)+(a,b,512)*(a,b,512)->(a,b,512)

        return text_proxy, text_scale
