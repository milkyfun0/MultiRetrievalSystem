...
class CLIPVisionViPEmbeddings(nn.Module):
    def __init__(self, config: CLIPVisionConfig, additional_vision_config=None):
        ...
        self.embed_dim = config.hidden_size
        self.image_size = config.image_size
        self.temporal_size = additional_vision_config.temporal_size # 12
        self.if_use_temporal_embed = additional_vision_config.if_use_temporal_embed # 1
        self.patch_size = config.patch_size
...
class CLIPAttention(nn.Module):
    ...
    def forward(
        ...
        if inputs_size is not None:
            output, attn = self.forward2(hidden_states, inputs_size) #,None
            return output, attn
    ...
    def forward2(self, hidden_states, inputs_size):
        ...
        # get query proj
        query_states = self.q_proj(hidden_states) * self.scale
        key_states = self._shape(self.k_proj(hidden_states), -1, bsz) # ->(b,nh,M+N*L,hd)
        value_states = self._shape(self.v_proj(hidden_states), -1, bsz) # ->(b,nh,M+N*L,hd)

        proj_shape = (bsz * self.num_heads, -1, self.head_dim)
        query_states = self._shape(query_states, tgt_len, bsz).view(*proj_shape) # ->(b,nh,M+N*L,hd)->(bxnh,M+N*L,hd)
        key_states = key_states.view(*proj_shape) # ->(bxnh,M+N*L,hd)
        value_states = value_states.view(*proj_shape) # ->(bxnh,M+N*L,hd)
        ...
        # (b*nh*N, L, hd) x (b*nh*N, hd, M+L) -> (b*nh*N, L, M+L)
        # 即给定帧query patch token要和所有带上M个proxy token的帧内的patch token进行自注意力分数计算
        attn_weights = nn.functional.softmax(attn_weights, dim=-1)
        attn_probs = nn.functional.dropout(attn_weights, p=self.dropout, training=self.training)
        attn_output = torch.bmm(attn_probs, v)   # [B*num_heads*N, L, head_dim]
        # (b*nh*N, L, M+L) x (b*nh*N, M+L, hd) -> (b*nh*N, L, hd)
        # 即帧内的给定patch token，加上proxy token的帧内所有patch token的注意力分数和这些帧内的包含proxy token的所有帧进行加权重新聚合成L个token
        ...
        # 当前的每个视频的每个帧都和proxy token有交互，但这些proxy token是复制到每个帧下再交互的
        ...
        attn_weights = torch.bmm(q, k.transpose(1, 2))
        # (b*nh,M,hd) x (b*nh,hd,M+N*L) -> (b*nh,M,M+N*L)
        # 即当前视频下的proxy token 要和 当前视频的包含proxy token的所有patch token进行注自意力分数计算
        attn_weights = nn.functional.softmax(attn_weights, dim=-1)
        attn_probs = nn.functional.dropout(attn_weights, p=self.dropout, training=self.training)
        attn_output = torch.bmm(attn_probs, v)   # [B*num_heads, M, head_dim]
        # (b*nh,M,M+N*L) x (b*nh,M+N*L,hd) -> (b*nh,M,hd)
        # 即当前video下的每个proxy token和所有token的注意力分数作为权重，聚合所有token为M个token
        attn_output = attn_output.view(bsz, self.num_heads, M, self.head_dim)
        attn_output = attn_output.transpose(1, 2)
        attn_output_cls = attn_output.reshape(bsz, M, embed_dim)  # [B, M, C]

        attn_output = torch.cat([attn_output_cls, attn_output_frames], dim=1) # (b,M+N*L,dim)

        attn_output = self.out_proj(attn_output)
        attn_weights = attn_weights.view(bsz, self.num_heads, M, -1).permute(0,2,3,1).sum(-1) # -> (b,nh,M,M+N*L)->(b,M,M+N*L,nh)->(b,M,M+N*L)
...
class CLIPEncoder(nn.Module):
    ...
    def forward(
        ...
        hidden_states = inputs_embeds # (b,M+N*L,dim)
        for idx, encoder_layer in enumerate(self.layers):
            ...
            hidden_states = layer_outputs[0] # (b,M+N*L,dim)
...
class CLIPVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        hidden_states, inputs_size = self.embeddings(pixel_values)   # tuple: ( [B,M+N*L,dim], (M,N,L))
        ...
        last_hidden_state = encoder_outputs[0] # (b,M+N*L,dim)
        pooled_output = last_hidden_state[:, 0, :] # 取全局的第0个proxy token表示这个视频
        pooled_output = self.post_layernorm(pooled_output) # (b,M+N*L,dim)

        if not return_dict:
            return (last_hidden_state, pooled_output) + encoder_outputs[1:]

        return BaseModelOutputWithPooling(
            last_hidden_state=last_hidden_state, # (b,M+N*L,dim)
            pooler_output=pooled_output, # (b,dim)
            hidden_states=encoder_outputs.hidden_states,
            attentions=encoder_outputs.attentions,
        )
...
class CLIPModel(CLIPPreTrainedModel):
    ...
    def __init__(self, config: CLIPConfig):
        ...
        self.visual_projection_patch = nn.Linear(self.vision_embed_dim, self.projection_dim, bias=False)
    ...
    def forward(
        ...
        image_embeds = vision_outputs[1] # pooled: (b,dim)
        image_embeds = self.visual_projection(image_embeds)

        text_embeds = text_outputs[1] # # pooled: (b,dim)
        text_embeds = self.text_projection(text_embeds)

        vision_patch_embeds = vision_outputs[0]
        vision_patch_embeds = self.visual_projection_patch(vision_patch_embeds)
        vision_outputs['last_hidden_state'] = vision_patch_embeds

        # normalized features
        image_embeds = image_embeds / image_embeds.norm(dim=-1, keepdim=True) # (b,dim)
        text_embeds = text_embeds / text_embeds.norm(dim=-1, keepdim=True) # (a,dim)
