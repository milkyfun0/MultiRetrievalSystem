...
class ImageTextEvalDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_path = image_path.replace("/home/sx1025/MedicalRetrieval/data", "/Users/caoqixuan.1/Code/MACCAI")
        ...
        if self.name == "chexpert5x200":
            ...
            text = eval(self.df["text"][index])[0]


        sample = {"images": image, "text": text, "image_path": image_path}
    ...
    def collate_fn(self, instances: List):
        ...
        text_tokens = self.tokenizer(
            collate["text"], padding="max_length", truncation=True, return_tensors="pt", max_length=self.text_max_length
        )
        collate["text_tokens"] = text_tokens
        texts = list([ins["text"] for ins in instances])
        collate["texts"] = texts
        collate["image_paths"] = list([ins["image_path"] for ins in instances])
