...
class CXRClip(nn.Module):
    def __init__(self, model_config: Dict, all_loss_config: Dict, tokenizer: PreTrainedTokenizer = None):
        ...
        self.visual_bias = torch.nn.Parameter(
            torch.rand(1, model_config["projection_head"]["proj_dim"]),
            requires_grad=True
        )
        self.text_bias = torch.nn.Parameter(
            torch.rand(1, model_config["projection_head"]["proj_dim"]),
            requires_grad=True
        )
        ...
        if self.projection:
            ...
            self.image_token_projection = load_projection_head(
                embedding_dim=self.image_encoder.out_dim, config_projection_head=model_config["llama_projection_head"]
            )
            self.text_token_projection = load_projection_head(
                embedding_dim=self.text_encoder.out_dim, config_projection_head=model_config["llama_projection_head"]
            )
            self.image_linear_proj = torch.nn.Sequential(
                nn.Linear(model_config["projection_head"]["proj_dim"],
                          model_config["projection_head"]["proj_dim"] // 4),
                nn.Linear(model_config["projection_head"]["proj_dim"] // 4, 512),
            )
            ...
            self.llama_proj = torch.nn.Sequential(
                nn.Linear(
                    model_config["projection_head"]["proj_dim"], model_config["projection_head"]["proj_dim"]),
            )
    ...
    def pre_encode(self, batch, device=None):
        ...
        image_embeddings = llama_image_embeddings + image_embeddings
        text_embeddings = llama_text_embeddings + text_embeddings
    ...
    def llama_encode(self, image_tokens, text_tokens, attention_mask):
        ...
        visual_prompt_expanded = self.visual_prompt.expand(image_tokens.size(0), -1, -1)
        llama_image_tokens = torch.cat([visual_prompt_expanded, image_tokens], dim=1)
        ...
        text_prompt_expanded = self.text_prompt.expand(text_tokens.size(0), -1, -1)  # 扩展到 batch 大小
        llama_text_tokens = torch.cat([text_prompt_expanded, text_tokens], dim=1)  # 拼接

        attention_mask = torch.cat(
            [torch.ones(text_prompt_expanded.size(0), 1, device=attention_mask.device), attention_mask], dim=1
        )
    ...
    def forward(self, batch, device=None):
        ...
        image_features_g, image_tokens = self.encode_image(batch["images"].to(device))
        # image_features_g = self.encode_image(batch["images"].to(torch.float16).to(device))
        text_features_g, text_tokens, attention_mask = self.encode_text(batch["text_tokens"].to(device))
        ...
        # image_embeddings = llama_image_embeddings
        ...
        out = {
            ...
            "llama_image_embeddings": llama_image_embeddings,
            "llama_text_embeddings": llama_text_embeddings,
        }
        # #
        ...
        # #
        # # image_view_encode = self.encode_image(batch["image_views"].to(torch.float16).to(device))
