...
class Evaluator:
    def __init__(self, config: Dict, ckpt_paths):
        ...
        self.model = build_model(
            model_config=self.ckpt_config["model"], loss_config=self.ckpt_config["loss"],
    ...
    def evaluate_clip(self, checkpoint, test_dataset_name):
        ...
        if test_dataset_name in {"chexpert5x200"}:
            results["zeroshot_gloria"] = self.zeroshot_gloria(image_embeddings, label_names, class_list, 1000000,
                                                              "mean")
        ...
        if test_dataset_name in {"chexpert5x200", "mimic_cxr", "openi", "arch"}:
            results["retrieval_i2t"] = retrieval_image_text(image_embeddings, text_embeddings, texts)
            results["retrieval_t2i"] = retrieval_text_image(image_embeddings, text_embeddings, texts)
            results["rsum"] = results["retrieval_i2t"]["sum"] + results["retrieval_t2i"]["sum"]
            print(results)
        return results
...
def retrieval_image_text(image_embeddings: np.ndarray, text_embeddings: np.ndarray, text_list: list = []):
    ...
    similarities = metrics.pairwise.cosine_similarity(image_embeddings, text_embeddings)  # n x m
    print(similarities.shape)

    result = i2t(similarities)
    result = {"Recall@1": result[0], "Recall@5": result[1], "Recall@10": result[2], "MeanRank": result[4],
...
def retrieval_text_image(image_embeddings: np.ndarray, text_embeddings: np.ndarray, text_list: list = []):
    log.info("evaluate text image retrieval")
    similarities = metrics.pairwise.cosine_similarity(text_embeddings, image_embeddings).T  # n x m
    result = t2i(similarities)
    result = {"Recall@1": result[0], "Recall@5": result[1], "Recall@10": result[2], "MeanRank": result[4],
              'sum': result[0] + result[1] + result[2]}
    return result
